...
public class FuelRefineryRecipeCategory implements IRecipeCategory<FuelRefineryRecipeWrapper> {

    private static final ResourceLocation TEXTURE = new ResourceLocation(Reference.MOD_ID,
        "textures/gui/container/fuel_refinery.png");
    ...
    public FuelRefineryRecipeCategory(IGuiHelper guiHelper) {
        this.background = guiHelper.createDrawable(TEXTURE, 0, 0, 177, 100, 177, 184);
        this.icon = guiHelper.createDrawableIngredient(new ItemStack(ModBlocks.FUEL_REFINERY));
        ...
        this.localizedName = I18n.format("tile.ad_astra.fuel_refinery.name");
    }

    @Nonnull
    @Override
    public String getUid() {
        return AdAstraJEIPlugin.FUEL_REFINERY_CATEGORY;
    }
    ...
    @Override
    public void setRecipe(@Nonnull IRecipeLayout recipeLayout, @Nonnull FuelRefineryRecipeWrapper recipeWrapper,
                          @Nonnull IIngredients ingredients) {
        recipeLayout.getItemStacks().init(0, true, 77, 52);
        recipeLayout.getFluidStacks().init(1, false, 100, 22, 12, 46, 10000, false, null);
