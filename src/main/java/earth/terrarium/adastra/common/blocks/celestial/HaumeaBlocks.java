...
public class HaumeaBlocks extends Block {
    public static final PropertyEnum<EnumHaumeaBlocks> TYPE = PropertyEnum.create("type", EnumHaumeaBlocks.class);
    ...
    public void getSubBlocks(CreativeTabs tab, NonNullList<ItemStack> list) {
        for (EnumHaumeaBlocks t : EnumHaumeaBlocks.values()) list.add(new ItemStack(this, 1, t.getMeta()));
    }
    ...
    @Override public Item getItemDropped(IBlockState state, Random rand, int fortune) {
        if (getMetaFromState(state) == 2) return ModItems.DOLOMITE_CRYSTAL;
    ...
    public enum EnumHaumeaBlocks implements IStringSerializable {
        HAUMEA_GRUNT(0, "haumea_grunt"), HAUMEA_SUBGRUNT(1, "haumea_subgrunt"),
        HAUMEA_DOLOMITE_ORE(2, "haumea_dolomite_ore"), HAUMEA_FROZEN_CRUST(3, "haumea_frozen_crust");
        private final int meta; private final String name;
        private static final EnumHaumeaBlocks[] VALUES = values();
        EnumHaumeaBlocks(int meta, String name) { this.meta = meta; this.name = name; }
    ...
    @Override public IBlockState getStateFromMeta(int meta) { return getDefaultState().withProperty(TYPE, EnumHaumeaBlocks.byMetadata(meta)); }
