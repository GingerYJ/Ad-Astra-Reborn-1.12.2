...
- `zh_cn.lang` is aligned with `en_us.lang`, and `zh_cn.json` is aligned with
  `en_us.json`, using the copied 1.20 Chinese strings plus current 1.12.2
  UI/HUD additions. The latest language pass covers Radio GUI, planet
  selection/space station text, TI-69/HUD strings, machine energy/fluid
  tooltips, side configuration text, Zip Gun propellant/help text, gas-tank
  tooltips, armor tags, and config section strings with no missing keys, extra
  keys, placeholder mismatches, duplicates, or malformed lines.
...
- Direct 1.12 crafting JSON coverage is now 270 files:
  ...
  - 15 low-risk Mercury terrain/decor recipes converted from generated 1.20
    ...
    `mercury_pillar`.
  - 15 low-risk Venus terrain/decor recipes converted from generated 1.20
    crafting data: `venus_stone_slab`, `venus_stone_stairs`,
    `venus_cobblestone_slab`, `venus_cobblestone_stairs`,
    `venus_stone_bricks`, `venus_stone_brick_slab`,
    `venus_stone_brick_stairs`, `venus_stone_brick_wall`,
    `polished_venus_stone`, `polished_venus_stone_slab`,
    `polished_venus_stone_stairs`, `chiseled_venus_stone_bricks`,
    `chiseled_venus_stone_slab`, `chiseled_venus_stone_stairs`, and
    `venus_pillar`.
  - 15 low-risk Glacio terrain/decor recipes converted from generated 1.20
    crafting data: `glacio_stone_slab`, `glacio_stone_stairs`,
    `glacio_cobblestone_slab`, `glacio_cobblestone_stairs`,
    `glacio_stone_bricks`, `glacio_stone_brick_slab`,
    `glacio_stone_brick_stairs`, `glacio_stone_brick_wall`,
    `polished_glacio_stone`, `polished_glacio_stone_slab`,
  ...
  - 12 low-risk Permafrost terrain/decor recipes converted from generated 1.20
    ...
    `permafrost_pillar`.
  - 20 low-risk metal decorative recipes converted from generated 1.20
  ...
  - 25 low-risk aeronos, strophar, and glacian wood-family recipes converted
...
- Hardcoded first-pass coverage that already exists:
  ...
  - `AdAstraChunkGenerator` creates a safe flat world: bedrock at y=0, filler
    from y=1 to y=62, and a surface block at y=63. Its first-pass `populate`
    path now adds deterministic Ad Astra ore veins into the filler layer.
...
- Completely unconverted or not represented by runtime logic:
  ...
  - Non-ore configured/placed feature generation, including Moon soul soil,
    Mars rock blobs, and Venus infernal spires.
  - All structure placement and lookup APIs. `generateStructures` returns false,
    and nearest/inside-structure queries return null.
...
Ore vein implementation snapshot, 2026-07-01:
...
- 1.12.2 runtime entry point:
  ...
  - `AdAstraChunkGenerator.populate(int chunkX, int chunkZ)` now runs the first
    Java-local ore generation batch. `generateChunk()` remains terrain-only.
...
- Current Java-local implementation:
  - A private `PlanetOreSpec` table in `AdAstraChunkGenerator` stores planet
    name, output `IBlockState`, vein size, count per chunk, clamped 1.12 Y range,
    and replaceable `Block` set.
  - `populate` creates a deterministic per-chunk `Random` from the world seed and
    chunk coordinates, filters specs by `PlanetDimensionProperties.getName()`,
    and places vanilla-style ellipsoid veins.
  - The replacement predicate only swaps the configured planetary filler blocks,
    keeping y=0 bedrock and y=63 surface intact. Moon specs can also target
    `MOON_DEEPSLATE`, although the current flat generator does not expose it.
  - 1.20 Y ranges are clamped into the current flat 1.12 terrain:
    `effectiveMinY = max(1, sourceMinY)` and
...
| Glacio | `glacio_copper_ore` | `GLACIO_COPPER_ORE` | 17 | 16 | -16..112 | 1..62 | Deferred from runtime generation until the 1.12 copper output policy is explicit. |
...
1. Runtime ore generation is started for Moon, Mars, Mercury, Venus, and
   non-copper/non-deepslate Glacio ore specs. Validate it in a dev world, then
...
5. Leave deepslate variants, large structures, and crater/noise terrain out of
   the next batch until the simple feature and NBT paths have smoke tests.
...
| `mogler` | Registered, spawn egg, generic hostile melee AI/attributes, fire immune, renderer now uses a first-pass 1.12 `ModelMogler` port with copied texture, source-like 0.7 shadow, four-leg walk animation, large body, head planes, snout, and tusks. | Hoglin-derived entity with `MoglerModel`, high health, knockback, and hoglin-style combat. | Runtime dev-client visual validation, hoglin-style AI/knockback, source attributes, drops/sounds. |
| `zombified_mogler` | Registered, spawn egg, generic hostile melee AI/attributes, fire immune, renderer now uses the first-pass 1.12 `ModelMogler` port with copied zombified texture, source-like 0.7 shadow, and four-leg walk animation. | Zoglin-derived entity rendered with `MoglerModel` and zombified texture. | Runtime dev-client visual validation, zoglin-style AI/knockback, source attributes, drops/sounds. |
...
| `glacian_ram` | Registered, spawn egg, generic neutral melee placeholder, renderer now uses a first-pass 1.12 `ModelGlacianRam` port with copied normal texture, source-like 0.7 shadow, four-leg walk animation, ears, horns, body planes, and head yaw/pitch. | Animal and `Shearable` entity with `GlacianRamModel`, sheared texture, ice-shard food, breeding, milking, permafrost eating, neck/head animation. | Runtime dev-client visual validation, real animal behavior, sheared state/data sync, sheared texture selection, shearing/milking/breeding, eat-permafrost goal and neck/head eating animation, sounds. |
